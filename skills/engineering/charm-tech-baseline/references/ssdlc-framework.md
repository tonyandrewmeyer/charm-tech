...
- GRC / OCISO — `security@canonical.com`; weekly SSDLC Office Hours;
  `~SSDLC` Mattermost.
- Engineering Excellence — TIOBE TICS.
- Security Engineering — `~SBOM`, `~Security` (SBOM / secscan).
- PSIRT — `security@ubuntu.com`.

## SSDLC Jira automation

Per-product SSDLC Epics + tasks are generated automatically from the
product type + planned release type. Do not hand-roll an SSDLC backlog
— run the automation.

## Exceptions

Per-cycle only. Submitted via the SECQ Jira form; require CISO
approval; not granted for lack of resources.
